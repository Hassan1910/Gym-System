...
// Handle the accept and decline actions
if (isset($_POST['accept'])) {
    $member_id = $_POST['member_id'];
    $update_status_query = "UPDATE members SET status='Active' WHERE user_id='$member_id'";
    mysqli_query($con, $update_status_query);
} elseif (isset($_POST['decline'])) {
    $member_id = $_POST['member_id'];
    $update_status_query = "UPDATE members SET status='Declined' WHERE user_id='$member_id'";
           ...
           <tbody> 
                ...
                <td><div class='text-center'>
                  <?php if ($row['status'] == 'Active') { 
                      ...
                      echo '<i class="fas fa-circle" style="color:gray;"></i> Declined'; 
                  } ?>
                </div></td>
                <td><div class='text-center'>
                  <?php if ($row['status'] == 'Pending') { ?>
                      <form method="POST" action="member-status.php" style="display:inline;">
                          <input type="hidden" name="member_id" value="<?php echo $row['user_id']; ?>" />
                          <button type="submit" name="accept" class="btn btn-success btn-mini">Accept</button>
                      </form>
                      <form method="POST" action="member-status.php" style="display:inline;">
                          <input type="hidden" name="member_id" value="<?php echo $row['user_id']; ?>" />
                          <button type="submit" name="decline" class="btn btn-danger btn-mini">Decline</button>
                      </form>
                  <?php } ?>
                </div></td>
                </tr>
